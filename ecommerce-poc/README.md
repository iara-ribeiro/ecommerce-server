...
```bash
ecommerce-poc$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ecommerce-poc$ sam local start-api
ecommerce-poc$ curl http://localhost:3000/
```
...
```bash
ecommerce-poc$ sam logs -n HelloWorldFunction --stack-name ecommerce-poc --tail
```
...
```bash
aws cloudformation delete-stack --stack-name ecommerce-poc
```
